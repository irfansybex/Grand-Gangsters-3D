...
[ExecuteInEditMode]
public class EditItemBlock : MonoBehaviour
{
	public bool runFlag;
	...
	public void EditBuilding()
	{
		...
		for (int i = 0; i < mapData.blockLine.Length; i++)
		{
			for (int j = 0; j < mapData.blockLine[i].blockLine.Length; j++)
			{
				...
				mapData.blockLine[i].blockLine[j].itemList = null;
				if (list.Count <= 0)
				{
					continue;
				}
				mapData.blockLine[i].blockLine[j].itemList = new BuildingInfo[CountBuildingNum(list)];
				num = 0;
				for (int k = 0; k < list.Count; k++)
				{
					mapData.blockLine[i].blockLine[j].itemList[num] = new BuildingInfo();
					mapData.blockLine[i].blockLine[j].itemList[num].position = list[k].transform.position;
					mapData.blockLine[i].blockLine[j].itemList[num].rotation = list[k].transform.eulerAngles;
					mapData.blockLine[i].blockLine[j].itemList[num].index = GetBuildingIndex(list[k].name);
					num++;
					if (list[k].transform.childCount > 0)
					{
						...
						{
							mapData.blockLine[i].blockLine[j].itemList[num] = new BuildingInfo();
							mapData.blockLine[i].blockLine[j].itemList[num].position = list[k].transform.GetChild(l).position;
							mapData.blockLine[i].blockLine[j].itemList[num].rotation = list[k].transform.GetChild(l).eulerAngles;
							mapData.blockLine[i].blockLine[j].itemList[num].index = GetBuildingIndex(list[k].transform.GetChild(l).gameObject.name);
							num++;
						}
					}
				}
