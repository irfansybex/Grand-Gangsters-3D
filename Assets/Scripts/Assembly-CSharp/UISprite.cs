...
public class UISprite : UIWidget
{
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			Vector4 result = new Vector4((mDrawRegion.x != 0f) ? Mathf.Lerp(num, num3, mDrawRegion.x) : num, (mDrawRegion.y != 0f) ? Mathf.Lerp(num2, num4, mDrawRegion.y) : num2, (mDrawRegion.z != 1f) ? Mathf.Lerp(num, num3, mDrawRegion.z) : num3, (mDrawRegion.w != 1f) ? Mathf.Lerp(num2, num4, mDrawRegion.w) : num4);
			float num11 = minWidth;
			float num12 = minHeight;
			if (result.z - result.x < num11)
			{
				float num13 = (result.x + result.z) * 0.5f;
				result.x = Mathf.Round(num13 - num11 * 0.5f);
				result.z = result.x + num11;
			}
			if (result.w - result.y < num12)
			{
				float num14 = (result.y + result.w) * 0.5f;
				result.y = Mathf.Round(num14 - num12 * 0.5f);
				result.w = result.y + num11;
			}
			return result;
		}
	}
